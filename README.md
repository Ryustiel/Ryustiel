...
- RAGraph — Relational Accessor Graph  
  ...
  Dynamic, multi‑layer knowledge graph for RAG that retrieves context via weighted semantic “accessors”
  over PostgreSQL + pgvector—great for messy, LLM‑extracted knowledge bases.  
...
- Automated Email & Document Processing Pipeline (LLM Storage Service)  
  ...
  Async, containerized pipeline that ingests emails, OCRs attachments, and summarizes content with GPT;
  pluggable content‑addressable storage (Local, S3, Google Drive).  
...
- Spark + PyTorch: Distributed LSTM for Time‑Series Forecasting  
  ...
  End‑to‑end workflow using Spark for large‑scale preprocessing and PyTorch DDP via TorchDistributor
  for distributed LSTM training on time‑series data.  
  Tech: Apache Spark, PyTorch (DDP), TorchDistributor, PyArrow, Docker.

- Gemini ImageChat  
  https://github.com/Ryustiel/GoogleGenAI  
  A Streamlit app to chat with Google’s Gemini image model: attach local images and receive generated
  images in the conversation; outputs are auto‑saved to project folders.  
...
- Aym — Conversational Graph Builder  
  ...
  Turn LLM debates into a living knowledge graph with tool‑proposed node/edge edits and human confirmation,
  rendered live in an interactive 3D Plotly view; persisted as a single JSON graph.  
